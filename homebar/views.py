...
class SpiritDetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            user_has_spirit_in_collection = UserCollection.objects.filter(
...
class SpiritCreateView(
    LoginRequiredMixin, PermissionRequiredMixin, generic.edit.CreateView
):
...
class CocktailDetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            user = self.request.user
            spirit_in_user_collection = UserCollection.objects.filter(
                user=user
            ).values_list("spirit__subcategory", flat=True)
        else:
...
def spirit_search(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "homebar/spirit_search.html",
            {"searched": searched, "spirits": spirits},
        )
...
def cocktail_search(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "homebar/cocktail_search.html",
            {"searched": searched, "cocktails": cocktails},
        )
...
def category_detail(request, category_id):
    ...
    return render(
        request,
        "homebar/category_detail.html",
        {"category": category, "spirits_in_category": spirits_in_category},
    )
...
def add_to_collection(request, spirit_id):
    if request.method == "GET":
        ...
        if not UserCollection.objects.filter(
            ...
            messages.info(
                request, "You already have this spirit in your collection!"
            )
...
def remove_from_collection(request, spirit_id):
    if request.method == "POST":
        ...
        try:
            ...
            messages.info(
                request, "You don't have this spirit in your collection!"
            )
    spirit_detail_url = reverse("spirit_detail", args=[spirit_id])
    return redirect(spirit_detail_url)
...
def view_collection(request):
    if request.method == "GET":
        ...
        return render(
            request,
            "homebar/spirit_collection.html",
            {
                "collection": collection,
                "suggested_cocktail": suggested_cocktail,
            },
        )
    ...
    if request.method == "POST":
        ...
        if commented:
            comment = Comment.objects.create(
                text=commented, author=request.user
            )
...
def spirit_comment_delete(request, spirit_id, comment_id):
    ...
    if request.method == "POST":
        if (
            ...
            or comment.author.username == request.user.username
        ):
            comment.delete()
    ...
    if request.method == "POST":
        ...
        if rated:
            ...
            user_rating = Rating.objects.create(
                star_rating=star_rating, author=request.user
            )
...
def cocktail_category_detail(request, cocktailcategory_id):
    cocktail_category = get_object_or_404(
        CocktailCategory, id=cocktailcategory_id
    )
    cocktails_in_category = Cocktail.objects.filter(
        cocktail_category=cocktail_category
    )
    return render(
        request,
        "homebar/cocktailcategory_detail.html",
        {
            "cocktail_category": cocktail_category,
            "cocktails_in_category": cocktails_in_category,
        },
    )
